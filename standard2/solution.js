...
const assignProfessionAndSeniority = () => {
...
const createTableRows = (data) => {
  const trsArray = data.map(
    (dataItem, index) => `
    <tr>
     <td>${index}</td>
     <td>${dataItem.name}</td>
     <td>${dataItem.profession}</td>
     <td>${dataItem.salary}</td>
     <td><button style="padding: 5px; margin: 0">&times;</button></td>
</tr>
    `
  );
  return trsArray.join("");
};
...
const renderHTML = (htmlStructure, elementId) => {
  document.getElementById(elementId).innerHTML = htmlStructure;
};

const generateStaffButton = document.getElementById("generateStaff");
const assingRolesButton = document.getElementById("assingRoles");
const renderStaffButton = document.getElementById("renderStaff");

generateStaffButton.addEventListener("click", () => {
  createStaff();
});

assingRolesButton.addEventListener("click", () => {
  assignProfessionAndSeniority();
});

renderStaffButton.addEventListener("click", () => {
  const trs = createTableRows(staff);
  renderHTML(trs, "output");
});
...
class Pracownik {
  constructor(imie, profesja, oczekiwaniaFinansowe) {
    this.imie = imie;
    this.profesja = profesja;
    this.oczekiwaniaFinansowe = oczekiwaniaFinansowe;
  }

  informacje() {
    return `Imię: ${this.imie}, Profesja: ${this.profesja}, Oczekiwania finansowe: ${this.oczekiwaniaFinansowe}`;
  }
}

function utworzPracownika(e) {
  e.preventDefault();
  const formularz = document.getElementById("addWorker");
  const formData = new FormData(formularz);

  const imie = formData.get("name");
  const profesja = formData.get("profession");
  const oczekiwaniaFinansowe = formData.get("salary");

  const nowyPracownik = new Pracownik(imie, profesja, oczekiwaniaFinansowe);

  // alert(nowyPracownik.informacje());
...
document
  .getElementById("addWorker")
  .addEventListener("submit", utworzPracownika);
